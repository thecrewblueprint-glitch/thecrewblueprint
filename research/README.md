...
The ordered list below preserves the original early handoff sequence through Package 21; it is not the complete current catalog. Use the [corpus run log](RESEARCH_CORPUS_RUN_LOG_2026-08-30.md) for all 60 pre-queue records, semantic collision keys, and current-versus-superseded decisions. Packages 43–45 are the current deeper Course 2 reruns. Packages 46–50 support five owner-review builds preserved on `curriculum/owner-audit-2026-08-30` and published as labeled, `noindex` audit copies; the named practitioner and observed-practice gates remain open.
