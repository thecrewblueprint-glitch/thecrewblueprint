...
class MockElement {
  ...
  set innerHTML(value) {
    ...
    const age = new MockElement(this.document, 'input');
    ...
    this.nodes.set('#cb-consent-age', age);
...
const age = backdrop.querySelector('#cb-consent-age');
...
assert.equal(age.checked, false, 'age confirmation must start unchecked');
...
age.checked = true;
age.trigger('change');
assert.equal(submit.disabled, true, 'age confirmation alone must not unlock the course');
terms.checked = true;
terms.trigger('change');
assert.equal(submit.disabled, true, 'two confirmations must not unlock the course');
safety.checked = true;
safety.trigger('change');
assert.equal(submit.disabled, false, 'all three confirmations should unlock the entry button');
...
assert.equal(savedRecord.version, '2026-08-30.3');
assert.equal(savedRecord.ageMajorityConfirmed, true);
...
console.log('- adult eligibility and dual unchecked acknowledgments enforced');
