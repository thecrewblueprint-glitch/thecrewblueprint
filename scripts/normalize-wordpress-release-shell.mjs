...
for(const file of (await walk(siteDir)).filter(file=>file.endsWith('.html'))){
  ...
  if(!html.includes('wordpress-global-shell.css'))html=html.replace(/<\/head>/i,`  <link rel="stylesheet" href="${prefix}${cssRel}?v=20260914-1">\n</head>`);
  ...
  records.push({source,main_sha256:sha(afterMain),header_count:hc,footer_count:fc,logo_refs:lc});
}
await mkdir(path.join(siteDir,'data','generated'),{recursive:true});
await writeFile(path.join(siteDir,manifestRel),JSON.stringify({version:'2026-09-14.1',documents:records.length,content_main_preserved:failures.every(item=>!item.includes('<main>')),records},null,2)+'\n','utf8');
