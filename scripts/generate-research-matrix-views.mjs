...
const supersededContent=content.filter((row)=>row.supersedes_content_id||row.superseded_by_content_id);const supersededSources=sources.filter((row)=>row.supersedes_source_id||row.superseded_by_source_id||row.source_status!=='active');
...
const domainRows=domains.map((domainId)=>{const domainContent=content.filter((row)=>row.domain_id_primary===domainId);const currentCourses=courseInventory.filter((course)=>course.domain_id_primary===domainId&&course.inventory_state==='canonical_current');const plannedCourses=domainContent.filter((row)=>row.content_type==='course'&&!courseInventory.some((course)=>course.course_id===row.course_id));const claims=domainContent.filter((row)=>['claim','boundary'].includes(row.content_type));const contentCompetencies=[...new Set(competencyEdges.filter((edge)=>domainContent.some((row)=>row.content_id===edge.content_id)).map((edge)=>edge.competency_id))];const researchCompetencies=[...new Set(researchEdges.filter((edge)=>edge.display_on_owner_map!==false).map((edge)=>edge.competency_id))];const blocked=reviews.filter((review)=>review.disposition==='blocked'&&domainContent.some((row)=>row.content_id===review.content_id_or_domain_id)).length;const revise=reviews.filter((review)=>review.disposition==='revise'&&(review.content_id_or_domain_id===domainId||domainContent.some((row)=>row.content_id===review.content_id_or_domain_id))).length;return{domainId,currentCourses,plannedCourses,claims,contentCompetencies,researchCompetencies,blocked,revise};});
writeView('domain-completeness.md','Domain Completeness / Planned-vs-Built View','Owner-facing structural view distinguishing built routes, planned content, matrixed claims, and structure-neutral research coverage.',domainRows.length ? `| Domain | Built routes | Planned nodes | Claims/boundaries | Content competencies | Research competencies (global pool) | Blocked | Revise |\n|---|---:|---:|---:|---:|---:|---:|---:|\n${domainRows.map((row)=>`| ${esc(row.domainId)} | ${row.currentCourses.length} | ${row.plannedCourses.length} | ${row.claims.length} | ${row.contentCompetencies.length} | ${row.researchCompetencies.length} | ${row.blocked} | ${row.revise} |`).join('\n')}` : 'No domain CONTENT records are present.');
...
fs.mkdirSync(viewsDir,{recursive:true});fs.writeFileSync(path.join(viewsDir,'README.md'),index,'utf8');
