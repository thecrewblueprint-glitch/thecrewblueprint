...
- 8 of 44 courses (Audio, Video, Staging & Carpentry, Backline Course 1/2) still have no `boundary` scope-disclaimer field, while the equivalent Lighting/Electrics courses do. Not yet fixed — flagged in §8, open.
- Two data fields, `tierClass` and `packet`, are collected on every course record but never rendered anywhere in this concept. Not yet fixed — flagged in §8, open.
- ~1MB of the file's 1.66MB is base64-embedded copies of images that already exist as separate files in `/images/` (the logo is embedded twice). Not yet fixed — flagged in §8, open.
...
**Still open, not part of this pass**: the 8 missing boundary fields, the image-duplication bloat, the two dead data fields, and the separately-discussed tier-naming reconciliation (Course 1/2/3 vs. real L2/L1-style rank names) — that last one is a distinct, larger structural question still awaiting the owner's decision and is not addressed by any fix in this section.
